 ...
 * <p>
 * 2026-09-16 sensei 指派「魔源和魔力的输入检测 + 修一部分无法识别的配方」。Botania 这边标准 API
 ...
 *   <li><b>花药台（实现类 {@code vazkii.botania.common.crafting.PetalsRecipe}）</b>：
 *       {@code getIngredients()} 只报花瓣，**试剂（任意种子）在 {@code getReagent()} 里**；
 *       另外还固定要**一桶水（1000 mB）**（Botania 把水存成 EMPTY/WATER/LAVA 三态，配方里没有 mB 字段）</li>
 *   <li><b>凝矿兰（orechid / orechid_ignem）</b>：输入与输出都是 Botania 自有的
 *       {@code StateIngredient}（**方块状态**，不是 Ingredient），标准 API 完全看不到 →
 *       这就是 sensei 报的「无法识别」。产出按 {@code weight / 同类型总权重} 当概率；
 *       魔力是机器硬编码常量（{@code OrechidBlockEntity.COST}，字节码里是 17500）</li>
 * </ol>
 */
public final class BotaniaCompat {
    ...
    /** 凝矿兰方块的类名（取它的 mana 常量用） */
    private static final String ORECHID_FLOWER = "vazkii.botania.common.block.flower.functional.OrechidBlockEntity";
    private static final String ORECHID_IGNEM_FLOWER =
            "vazkii.botania.common.block.flower.functional.OrechidIgnemBlockEntity";
    ...
    /** 凝矿兰总权重：按配方类型（botania:orechid / botania:orechid_ignem）分别求和，换管理器就清空 */
    ...
    // ── 凝矿兰（orechid / orechid_ignem）──

    /** 是不是凝矿兰配方（{@code botania:orechid} 或 {@code botania:orechid_ignem}） */
    public static boolean isOrechid(@Nullable Recipe<?> recipe) {
        if (recipe == null) return false;
        String name = recipe.getClass().getName();
        return ORECHID_RECIPE.equals(name) || ORECHID_IGNEM_RECIPE.equals(name);
    }

    /** 凝矿兰输入（方块状态 → 物品，通常是石头 / 下界岩） */
    public static List<List<GenericStack>> orechidInputs(@Nullable Recipe<?> recipe) {
        return orechidSides(recipe, "getInput");
    }

    /** 凝矿兰产出（方块状态 → 物品，如煤矿石） */
    public static List<GenericStack> orechidOutputs(@Nullable Recipe<?> recipe) {
        var slots = orechidSides(recipe, "getOutput");
        return slots.isEmpty() ? List.of() : slots.get(0);
    }

    /** 配方权重（{@code getWeight()}） */
    public static int orechidWeight(@Nullable Recipe<?> recipe) {
        if (recipe == null) return -1;
    ...
    /**
     * 同类型（{@code botania:orechid} / {@code botania:orechid_ignem}）**全部配方的权重和** ——
     * 单条配方的概率 = 自身权重 / 这个总和。
     * <p>
     * 拿不到配方管理器（比如多人游戏客户端）→ 返回 -1（概率未知）。
     * 注意：**不能**用 Botania 自己的 {@code OrechidManager}，它是服务端 reload 监听器。
     */
    public static int orechidTotalWeight(@Nullable Recipe<?> recipe) {
        if (recipe == null) return -1;
        ...
        for (var other : manager.getRecipes()) {
            if (other == null || !isOrechid(other)) continue;
            if (!type.equals(String.valueOf(other.getType()))) continue;
            int weight = orechidWeight(other);
    ...
    /**
     * 凝矿兰每次转化的 mana 消耗：机器硬编码常量（{@code OrechidBlockEntity.COST} / {@code OrechidIgnemBlockEntity.COST}，
     * 字节码里是 17500）。取不到 → 0（= 不加魔力槽）。
     */
    public static int orechidManaCost(@Nullable Recipe<?> recipe) {
        if (recipe == null) return 0;
        String flower = ORECHID_IGNEM_RECIPE.equals(recipe.getClass().getName())
                ? ORECHID_IGNEM_FLOWER : ORECHID_FLOWER;
    ...
    /** 配方某一侧的 StateIngredient → 槽（方块状态列表 → 物品候选） */
    private static List<List<GenericStack>> orechidSides(@Nullable Recipe<?> recipe, String getter) {
        if (recipe == null) return List.of();
        var options = stateItems(invokeNoArg(recipe, getter));
